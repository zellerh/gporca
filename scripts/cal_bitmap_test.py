...
def parseargs():
    ...
    parser.add_argument("tests", metavar="TEST", choices=[[], "all", "none", "bitmap_scan_tests", "btree_scan_tests", "bitmap_join_tests"],
                        nargs="*",
                        help="Run these tests (all, bitmap_scan_tests, btree_scan_tests, bitmap_join_tests), default is none")
...
def run_btree_index_scan_tests(conn, execute_n_times):
    ...
    run_one_bitmap_scan_test(conn,
                             ...
                             0,
                             glob_rowcount/20, # btree index scan is only supported up to 5% selectivity
                             noSetupRequired,
    ...
    run_one_bitmap_scan_test(conn,
                             "Btree Scan Test; NDV=100; selectivity_pct=10*parameter_value; count(*)",
                             0,
                             5,
                             noSetupRequired,
    ...
    # all full table scan, no crossover
    run_one_bitmap_scan_test(conn,
                             "Btree Scan Test; NDV=100; selectivity_pct=10*parameter_value; max(txt)",
                             0,
                             5,
                             noSetupRequired,
    ...
    run_one_bitmap_scan_test(conn,
                             "Btree Scan Test; NDV=10000; selectivity_pct=0.01*parameter_value; count(*); largeNDV test",
                             0,
                             300,
                             noSetupRequired,
                             parameterize_btree_index_10000_narrow,
...
def inspectExistingTables(conn):
    ...
    log_output("Determining row count of an existing table")
    sqlStr = "SELECT count(*) from cal_txtst"
    curs = dbconn.execSQL(conn, sqlStr)
    ...
    log_output("Determining whether existing table is append-only")
    ...
    for row in rows:
        if re.search("appendonly", row[0]):
            ...
            log_output("Existing fact table is append-only" % glob_rowcount)
