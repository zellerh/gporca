...
//		  aggregate (no group by columns) and count(C0) will be 0 for an empty
	...
	)
{
	...
	AddProjectNode(mp, pexprInner, pexprSubquery, &pexprPrjInner);
	...
	)
{
	...
	if (m_fEnforceCorrelatedApply || IMDFunction::EfsVolatile == pmdFunc->GetFuncStability())
	{
